...
infra_items = [
    ...
    "Google Chrome on Ocean - CDP port 18801 - hosts Derayah sessions + TickerChart feed",
...
dirs = [
    ...
    "/home/mino/.config/google-chrome/derayah-profile/  - Chrome persistent session",
]
pdf.bullet(dirs)
...
components = [
    ...
    ("derayah_keepalive.py", "Every 5 min", "Cron-driven (OpenClaw). Checks browser up, Derayah tab open, session not expired. Auto-starts Google Chrome if down. On session expiry, attempts login in priority order: (1) API refresh token injection - ~15s, no reCAPTCHA, no OTP; (2) CDP real Chrome login; (3) Playwright stealth login. Scrolls page to prevent idle timeout. Notifies Amin max 2x/day only if all three methods fail."),
    ...
    ("post_market_v2.py", "Post-market", "Runs at 15:35 Sun-Thu. Comprehensive post-market analysis with learning loop. Analyzes picks performance, finds missed opportunities, generates strategy recommendations. Posts report to TASI Execution group. Saves reports to /home/mino/tasi-exec/reports/, tracks learning progress in learning.json."),
    ("ws_logger.py", "Utility", "WebSocket price feed logger. Captures TickerChart WebSocket frames for debugging and audit."),
    ("start-chrome.sh", "Utility", "Bash script to start Google Chrome with CDP on port 18801, SOCKS5 proxy, single-process mode. Run after reboot or when browser is down."),
]
...
cron_oc = [
    ("Every 5 min", "derayah-keepalive", "ENABLED", "Keep Chrome + Derayah tab live; auto-login on session expiry"),
    ...
    ("Sun-Thu 15:35", "post-market-analysis", "ENABLED", "Run post_market_v2.py; analyze picks, find missed opportunities, post report"),
    ("Thu 22:00", "sharia-list-refresh", "ENABLED", "Refresh Sharia-compliant stock list from Saudi Exchange"),
    ("Every 30 min", "ocean-health-monitor", "ENABLED", "System infrastructure: RAM/disk/CPU checks (not trading-specific)"),
    ("Every 2 h", "auth-token-sync", "ENABLED", "System infrastructure: Claude CLI token sync (not trading-specific)"),
    ("Daily 04:00", "daily-ram-cleanup", "ENABLED", "System infrastructure: RAM cleanup (not trading-specific)"),
]
...
services = [
    ...
    ("chromium-derayah.service", "Google Chrome with CDP on port 18801 - Derayah sessions"),
...
steps = [
    ...
    ("After close", "Post-Market Analysis", "post_market_v2.py runs at 15:35. Analyzes all 269 Sharia stocks vs today's picks. Finds missed opportunities, generates strategy recommendations. Posts report to TASI Execution group. Saves report to reports/post_market_YYYY-MM-DD.html. Updates learning.json with sessions_analyzed, recommendations_made, missed_opportunities_avg."),
...
pdf.body(
    "Google Chrome uses /home/mino/.config/google-chrome/derayah-profile/ as user data dir. "
...
pdf.bullet([
    "Automatic: derayah-keepalive cron detects within 5 min, starts Google Chrome, attempts auto-login",
    "Manual via SSH: /home/mino/tasi-exec/start-chrome.sh (recommended) or google-chrome-stable --remote-debugging-port=18801 --user-data-dir=/home/mino/.config/google-chrome/derayah-profile/ &",
    "Manual via CRD: open Chrome Remote Desktop (PIN 056187), start Chrome manually",
])
...
pdf.subsection("Post-Market Analysis Not Running")
pdf.bullet([
    "Check: verify post-market-analysis cron is enabled (15:35 Sun-Thu)",
    "Check: ls /home/mino/tasi-exec/reports/ for daily reports",
    "Manual run: python3 /home/mino/tasi-exec/post_market_v2.py",
    "Check learning.json: cat /home/mino/tasi-exec/learning.json",
])
...
issues = [
    ...
    ("Keepalive: Browser Context Closed", "derayah_keepalive.py killed Google Chrome on session expiry, restarted it, then auto-login failed with 'Target page, context or browser has been closed' mid-login at 16:53. No NOTIFY sent - alert threshold not reached.", "No fix yet. Follow-up: add failure counter (3x) before NOTIFY; add CDP port readiness wait before Playwright connects after browser restart."),
]
...
pdf.multi_cell(0, 5,
    "NOTE: This document reflects the system state as of 2026-05-20. "
    "Changes to scripts, crons, or infrastructure should be logged in "
    "/home/mino/.openclaw/workspace/memory/issues.md and this document regenerated.",
    border=1, fill=True)

out = "/home/mino/tasi-exec/TASI_Ops_Procedure_v3_2026-05-20.pdf"
pdf.output(out)
