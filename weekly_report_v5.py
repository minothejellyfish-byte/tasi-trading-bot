...
"""
    ...
    # ── Send to Telegram via TASI Bot ──────────────────────────────
    try:
        import requests
        import os
        
        bot_token = os.environ.get("TASI_BOT_TOKEN", "")
        if not bot_token:
            print("[WARNING] TASI_BOT_TOKEN not set")
            return report
        chat_id = "-5235925419"  # TASI Execution group
        
        # Build summary message
        summary = f"📊 <b>TASI Weekly Report</b>\nWeek of {week_label}\n\n"
        summary += "<pre>"
        summary += "System     | Picks | Hit | PnL    | Win%\n"
        summary += "-----------|-------|-----|--------|------\n"
        for system_key, data in results.items():
            summary += f"{data['name']:<10} | {data['num_picks']:>5} | {data['hit_trades']:>3} | {data['total_pnl']:>+6.2f}% | {data['win_rate']:>5.1f}%\n"
        summary += "</pre>"
        ...
        if resp.status_code == 200:
            print("📤 Sent weekly report to TASI group")
        else:
